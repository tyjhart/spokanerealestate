...
Most people will want to know overall percentages, so we'll break those down by country, state, and then city. First, the breakdown by country:
...
We can summarize the data above with one statement: For the most part, taxpayers with local addresses own local parcels. Here are the other high points:

1. Almost all Spokane County parcels are owned by USA-addressed taxpayers (`r country_percentage$percentage[which(country_percentage$country == "USA")]`%)
1. `r round(state_percentage$percentage[which(state_percentage$state == "WA")], 2)`% of USA-addressed taxpayers have Washington state addresses. Next is California at `r round(state_percentage$percentage[which(state_percentage$state == "CA")], 2)`%.
